...
use super::{
    model::{
        invalid_state_error, DocumentFormat, DocumentFragment, DocumentId, DocumentStatus,
        LibraryError, LibraryErrorCode, LibraryResult, OcrBoundingBox, OcrJobId, OcrJobRecord,
        OcrModelStatus, OcrPoint, OcrTextBox, ScanJobState,
    },
...
impl LibraryService {
    ...
    pub(crate) fn cancel_ocr(&mut self, job_id: &OcrJobId) -> LibraryResult<OcrJobRecord> {
        ...
        if !matches!(
            job.state,
            ScanJobState::Queued | ScanJobState::Running | ScanJobState::Paused | ScanJobState::Failed
        ) {
            return Err(invalid_state_error(
                &[
                    ScanJobState::Queued,
    ...
    fn run_ocr_job_inner(
        ...
        let document = self.database.document_by_id(&job.document_id)?.ok_or_else(|| {
        ...
        let image = image::open(&input).map_err(|_| {
            LibraryError::new(
    ...
    fn store_text_layer(
        ...
        for (index, page) in pages.into_iter().enumerate() {
            if let Some(interrupted) = self.interrupted_ocr_job(&job.id)? { return Ok(interrupted); }
            self.database.replace_ocr_page(&job.document_id, page.page, page.source, &page.text, page.confidence, page.width, page.height, page.rotation_degrees, &page.boxes)?;
            self.database.add_ocr_metric(&job.id, Some(page.page), "text_layer", 0, 0)?;
            self.database.update_running_ocr_progress(&job.id, page_count, index as u32 + 1, 0)?;
        }
    ...
    fn run_scanned_pdf(
        ...
        if page_count == 0 {
            return Err(LibraryError::new(LibraryErrorCode::OcrCorruptDocument, "PDF has no pages"));
        }
        let work_dir = std::env::temp_dir().join(format!("moji-ocr-{}", job.id.0));
        fs::create_dir_all(&work_dir).map_err(|_| LibraryError::new(LibraryErrorCode::OcrPageFailed, "OCR workspace could not be created").retryable())?;
        let mut rendered = Vec::new();
        for page in 1..=page_count {
            let result = render_pdf_page(input, page, &work_dir);
            match result { Ok(image) => rendered.push((page, image)), Err(error) => { remove_ocr_workspace(&work_dir); return Err(error); } }
        }
    ...
    fn run_images(
        ...
        for (index, (page, image)) in images.into_iter().enumerate() {
            ...
            self.database.replace_ocr_page(&job.document_id, page, result.source, &result.text, result.confidence, result.width, result.height, result.rotation_degrees, &result.boxes)?;
            self.database.add_ocr_metric(&job.id, Some(page), "inference", page_started.elapsed().as_millis() as u64, model.bytes)?;
            let Some(_) = self.database.update_running_ocr_progress(&job.id, page_count, index as u32 + 1, 0)? else { return self.ocr_job(&job.id); };
        }
        self.complete_ocr_job(job, page_count, page_count, 0, started)
    }

    fn complete_ocr_job(&mut self, job: &OcrJobRecord, page_count: u32, processed: u32, failed: u32, started: Instant) -> LibraryResult<OcrJobRecord> {
        self.database.add_ocr_metric(&job.id, None, "total", started.elapsed().as_millis() as u64, job.model_bytes)?;
        self.database.update_ocr_job(&job.id, ScanJobState::Completed, page_count, processed, failed, job.retry_count, None, Some(started.elapsed().as_millis() as u64))
    }
...
impl PpOcrEngine {
    fn load(paths: &ModelPaths) -> LibraryResult<Self> {
        ...
        unsafe { std::env::set_var("ORT_DYLIB_PATH", &paths.runtime); }
        let mut engine = OcrLite::new();
        engine.init_models_no_angle(path_string(&paths.det)?, path_string(&paths.rec)?, path_string(&paths.dictionary)?, 2).map_err(model_error)?;
        if paths.orientation.exists() {
            let classifier = DocOrientationClassifier::from_path(&paths.orientation).map_err(model_error)?;
    ...
    fn recognize(&mut self, image: RgbImage) -> LibraryResult<OcrPage> {
        ...
        let output = self.engine.detect_with_options(&image, 10, 1920, 0.6, 0.3, 1.6, false, false, OcrOptions { use_doc_orientation: true, ..OcrOptions::default() }).map_err(model_error)?;
        let rotation_degrees = output.page_angle;
        let boxes = output.text_blocks.into_iter().filter_map(|block| {
            let text = block.text.trim().to_owned();
            (!text.is_empty()).then(|| OcrTextBox {
                text,
                confidence: block.text_score,
                bounding_box: OcrBoundingBox { points: block.box_points.into_iter().map(|point| unrotate_point(point.x, point.y, rotation_degrees, width, height)).collect() },
            })
        }).collect::<Vec<_>>();
        let text = boxes.iter().map(|item| item.text.as_str()).collect::<Vec<_>>().join("\n");
        let confidence = (!boxes.is_empty()).then(|| boxes.iter().map(|item| item.confidence).sum::<f32>() / boxes.len() as f32);
        Ok(OcrPage { page: 0, source: if boxes.is_empty() { "blank" } else { "ocr" }, text, confidence, width, height, rotation_degrees, boxes })
    }
}
...
fn model_paths(root: &Path) -> LibraryResult<ModelPaths> {
    ...
    let files = [
        ...
        ("runtime/onnxruntime.dll", root.join("runtime").join("onnxruntime.dll")),
    ];
    let missing = files.iter().filter_map(|(name, path)| (!path.is_file()).then(|| (*name).to_owned())).collect::<Vec<_>>();
    if !missing.is_empty() {
        return Err(LibraryError::new(LibraryErrorCode::OcrModelMissing, "local OCR model or CPU runtime is unavailable").retryable().with_details(serde_json::json!({ "missingAssets": missing, "modelDirectory": "ocr-models" })));
    }
    let bytes = files.iter().filter_map(|(_, path)| fs::metadata(path).ok().map(|metadata| metadata.len())).sum();
    Ok(ModelPaths { det: files[0].1.clone(), rec: files[1].1.clone(), dictionary: files[2].1.clone(), runtime: files[3].1.clone(), orientation: directory.join("orientation.onnx"), bytes })
}

fn extract_pdf_text_pages(path: &Path) -> LibraryResult<Vec<OcrPage>> {
    let document = PdfDocument::load(path).map_err(|_| LibraryError::new(LibraryErrorCode::OcrCorruptDocument, "PDF could not be read").retryable())?;
    let pages = document.get_pages();
    if pages.is_empty() { return Err(LibraryError::new(LibraryErrorCode::OcrCorruptDocument, "PDF has no pages")); }
    pages.into_keys().map(|page| {
        let text = document.extract_text(&[page]).unwrap_or_default();
        Ok(OcrPage { page, source: if has_effective_text(&text) { "text_layer" } else { "blank" }, text, confidence: None, width: 0, height: 0, rotation_degrees: 0, boxes: Vec::new() })
    }).collect()
}

fn has_effective_text(text: &str) -> bool {
    text.chars().filter(|character| character.is_alphanumeric() || ('\u{4e00}'..='\u{9fff}').contains(character)).count() >= 8
}

fn render_pdf_page(input: &Path, page: u32, work_dir: &Path) -> LibraryResult<RgbImage> {
    let prefix = work_dir.join(format!("page-{page}"));
    let output = Command::new("pdftoppm").args(["-f", &page.to_string(), "-l", &page.to_string(), "-r", "200", "-png"]).arg(input).arg(&prefix).output().map_err(|_| LibraryError::new(LibraryErrorCode::OcrPdfRendererUnavailable, "local PDF renderer is unavailable").retryable())?;
    if !output.status.success() { return Err(LibraryError::new(LibraryErrorCode::OcrPageFailed, "PDF page could not be rendered for OCR").retryable()); }
    let rendered = fs::read_dir(work_dir).map_err(|_| LibraryError::new(LibraryErrorCode::OcrPageFailed, "rendered PDF page could not be read").retryable())?.filter_map(Result::ok).map(|entry| entry.path()).find(|path| path.extension().and_then(|extension| extension.to_str()).is_some_and(|extension| extension.eq_ignore_ascii_case("png")) && path.file_stem().and_then(|name| name.to_str()).is_some_and(|name| name.starts_with(&format!("page-{page}-")))).ok_or_else(|| LibraryError::new(LibraryErrorCode::OcrPageFailed, "PDF renderer did not create a page image").retryable())?;
    image::open(&rendered).map_err(|_| LibraryError::new(LibraryErrorCode::OcrPageFailed, "rendered PDF page could not be decoded").retryable()).map(|image| image.to_rgb8())
}

fn hash_file(path: &Path) -> LibraryResult<String> {
    let bytes = fs::read(path).map_err(|_| LibraryError::new(LibraryErrorCode::OcrInputChanged, "document could not be read for OCR").retryable())?;
    Ok(Sha256::digest(bytes).iter().map(|byte| format!("{byte:02x}")).collect())
}

fn remove_ocr_workspace(work_dir: &Path) {
    let temp_root = std::env::temp_dir();
    if work_dir.starts_with(&temp_root) && work_dir.file_name().and_then(|name| name.to_str()).is_some_and(|name| name.starts_with("moji-ocr-")) {
        let _ = fs::remove_dir_all(work_dir);
    }
}

fn path_string(path: &Path) -> LibraryResult<&str> {
    path.to_str().ok_or_else(|| LibraryError::new(LibraryErrorCode::OcrModelInvalid, "OCR model path is not valid Unicode"))
}

fn model_error(error: impl std::fmt::Display) -> LibraryError {
    let message = error.to_string();
    let code = if message.to_ascii_lowercase().contains("dll") || message.to_ascii_lowercase().contains("runtime") { LibraryErrorCode::OcrRuntimeUnavailable } else { LibraryErrorCode::OcrModelInvalid };
...
fn unrotate_point(x: u32, y: u32, rotation_degrees: u32, width: u32, height: u32) -> OcrPoint {
    let (x, y) = match rotation_degrees {
        ...
        180 => (width.saturating_sub(1).saturating_sub(x), height.saturating_sub(1).saturating_sub(y)),
...
mod tests {
    ...
    fn reports_all_required_offline_assets_when_the_model_is_not_installed() {
        let root = std::env::temp_dir().join(format!("moji-ocr-model-missing-{}", std::process::id()));
        let error = model_paths(&root).expect_err("missing model assets must be explicit");
        assert_eq!(error.code, "OCR_MODEL_MISSING");
        let missing = error.details.unwrap()["missingAssets"].as_array().unwrap().len();
        assert_eq!(missing, 4);
    }
}
