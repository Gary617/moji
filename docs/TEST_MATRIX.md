...
| Tauri/WebView2 Office 宿主 POC | 隔离 Tauri 窗口加载 POC 页面，真实 WebView2 调试端点读取 24 样本结构化结果 | `pnpm test:editor-poc:tauri`；报告 `docs/editor-poc/results.tauri.{json,md}` | BLOCKED：当前环境未取得宿主矩阵结果；不计入浏览器 PASS |
